...
const Read = (props) => {
  ...
  if (error) {
    if (error.response && error.response.status === 404) {
      // console.group('components → child → [Read.js]')
  ...
  return (
    <Styled.read>
      ...
      <div className="group_signage" style={{ backgroundImage: `url(http://localhost:4000/uploads/${read.visual})` }}>
        <div className="outer_cell">
          <div className="inner_cell">
            <h4 className="title_signage">{read.subject}</h4>
            <p className="description_signage" dangerouslySetInnerHTML={{ __html: read.description }}></p>
          </div>
        </div>
      </div>
